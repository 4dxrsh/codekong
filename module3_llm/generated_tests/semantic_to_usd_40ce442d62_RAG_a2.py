...
def test_to_usd_precision():
    ...
    assert to_usd(1.0, 'USD') == 1.0, "Failed on one dollar value"
    assert to_usd(999.995, 'USD') == 1000.00, "Failed on large value with rounding down"
    assert to_usd(999.994, 'USD') == 999.99, "Failed on large value with rounding up"