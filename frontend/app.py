...
BADGE = {"syntactic": "#7c6bd6", "sdl": "#2f9e8f", "semantic": "#d68a2f",
         "higher_order": "#c2503f", "RAG": "#2f6bd6", "NO_RAG": "#8a8a8a"}

BASE = """<!doctype html><html><head><meta charset="utf-8">
<title>CodeKong — {{ title }}</title>
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.min.js"></script>
<style>
:root { --accent:#2f6bd6; --ink:#1c2333; --muted:#66708a; --bg:#f7f8fb; }
...
nav{background:#fff;border-bottom:1px solid #e3e6ee;padding:0 32px;display:flex;gap:26px;align-items:center;height:58px;position:sticky;top:0;z-index:5}
nav .brand{font-weight:700;color:var(--accent);font-size:19px;margin-right:12px}
nav a{color:var(--ink);text-decoration:none;font-size:15px;padding:4px 2px;border-bottom:2px solid transparent}
...
h1{font-size:30px;margin:.2em 0} h2{font-size:22px;margin-top:1.8em} h3{font-size:17px}
.card{background:#fff;border:1px solid #e3e6ee;border-radius:10px;padding:22px 26px;margin:18px 0}
.badge{display:inline-block;padding:1px 9px;border-radius:999px;color:#fff;font-size:12.5px;font-weight:600}
.grid2{display:grid;grid-template-columns:1fr 1fr;gap:18px}
table.data{width:100%;border-collapse:collapse;font-size:14.5px;background:#fff}
table.data th,table.data td{padding:8px 10px;border-bottom:1px solid #eceef4;text-align:left;vertical-align:top}
table.data th{background:#f0f2f8;position:sticky;top:58px}
tr.rowlink:hover{background:#f4f7ff;cursor:pointer}
pre,code,.mono{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
pre{background:#0f1420;color:#dde3f0;padding:14px 16px;border-radius:8px;overflow-x:auto;font-size:13.5px;line-height:1.5}
table.diff{width:100%;border-collapse:collapse;font-size:13px;background:#0f1420;color:#dde3f0;border-radius:8px;overflow:hidden}
table.diff td{padding:1px 10px;white-space:pre;font-family:ui-monospace,Menlo,Consolas,monospace}
table.diff td.ln{color:#5b6478;text-align:right;width:38px;user-select:none;border-right:1px solid #232a3d}
tr.del td.code{background:#4a1f26;} tr.add td.code{background:#173d2a;}
tr.del td.code::before{content:"- ";color:#e07a8a} tr.add td.code::before{content:"+ ";color:#5fce93}
.pass{color:#177245;font-weight:700}.fail{color:#b3261e;font-weight:700}
.pill-pass{background:#e3f4ea;color:#177245;padding:2px 10px;border-radius:999px;font-weight:600;font-size:13px}
.pill-fail{background:#fbe9e7;color:#b3261e;padding:2px 10px;border-radius:999px;font-weight:600;font-size:13px}
.muted{color:var(--muted)} .empty{padding:28px;text-align:center;color:var(--muted);background:#fff;border:1px dashed #ccd2e0;border-radius:10px}
.pipeline{display:flex;gap:10px;align-items:stretch;flex-wrap:wrap}
.stage{flex:1 1 140px;background:#fff;border:1.5px solid var(--accent);border-radius:9px;padding:10px 12px;font-size:13.5px;position:relative}
.stage b{display:block;color:var(--accent)} .arrow{align-self:center;color:var(--accent);font-size:20px}
details.ast{margin-left:16px;font-size:13.5px} details.ast>summary{cursor:pointer;padding:1px 4px;border-radius:4px}
details.ast>summary.marked{background:#ffe4b8;font-weight:700} details.ast>summary:hover{background:#eef2fc}
input[type=text],textarea,select{width:100%;padding:9px 11px;border:1px solid #ccd2e0;border-radius:7px;font-size:15px}
button,.btn{background:var(--accent);color:#fff;border:0;border-radius:7px;padding:10px 20px;font-size:15px;font-weight:600;cursor:pointer;text-decoration:none;display:inline-block}
.refs li{margin-bottom:10px;font-size:14.5px} .refs .why{color:var(--muted)}
.statgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:14px}
.stat{background:#fff;border:1px solid #e3e6ee;border-radius:10px;padding:14px;text-align:center}
...
<a href="/explore" class="{{ 'active' if page=='explore' }}">Explore Dataset</a>
<a href="/passed-tests" class="{{ 'active' if page=='passed' }}">Passed Tests</a>
<a href="/generate" class="{{ 'active' if page=='generate' }}">Generate Tests</a>
...
def badge(kind: str) -> str:
    color = BADGE.get(kind, "#66708a")
    return f'<span class="badge" style="background:{color}">{kind}</span>'
...
HOME = """
<h1>CodeKong</h1>
<p style="font-size:18px">RAG-conditioned test regeneration targeting <b>surviving mutants</b>:
does showing an LLM the actual codebase under test help it write tests that kill bugs the
existing suite misses — and does that help grow as the bugs get harder?</p>

<h2>What is mutation testing?</h2>
<div class="card">
<p>Mutation testing plants small artificial bugs ("mutants") in working code and checks whether
the test suite notices. Every mutant the suite fails to catch is a measured blind spot.</p>
<div class="grid2">
<pre># Original
...
<pre># Mutant (&gt;= becomes &gt;)
def is_valid_password(pw):
    return len(pw) <span style="background:#4a1f26;padding:0 4px">&gt;</span> 8</pre>
</div>
<p><i>If the test suite never checks a password of exactly length 8, this mutant survives —
and that survival is exactly the blind spot mutation testing exists to find.</i></p>
<table class="data"><tr><th>Class</th><th>What it does</th></tr>
<tr><td>{{ badge('sdl')|safe }} Statement deletion</td><td>Removes a statement outright (Python <code>ast</code>)</td></tr>
<tr><td>{{ badge('syntactic')|safe }} Syntax mutation</td><td>Swaps an operator or constant (<code>&gt;=</code>→<code>&gt;</code>), via mutmut</td></tr>
<tr><td>{{ badge('semantic')|safe }} LLM semantic mutation</td><td>An LLM injects a realistic, natural-looking bug rather than a fixed rule</td></tr>
<tr><td>{{ badge('higher_order')|safe }} Higher-order mutation</td><td>Combines two first-order mutants in one function (Jia &amp; Harman), simulating interacting faults</td></tr>
</table></div>

<h2>The CodeKong pipeline</h2>
...
<div class="stage"><b>2. Index</b>Embed function/docstring/test chunks into a local ChromaDB</div><div class="arrow">→</div>
<div class="stage"><b>3. Mutate</b>Four strategies generate candidate mutants</div><div class="arrow">→</div>
<div class="stage"><b>4. Filter</b>Run the existing suite; keep only <b>survivors</b></div><div class="arrow">→</div>
<div class="stage"><b>5. Generate</b>Top-K RAG chunks + diff + class-aware prompt → Ollama LLM → candidate test (one validator-guided retry)</div><div class="arrow">→</div>
<div class="stage"><b>6. Validate & log</b>Pass-on-original AND fail-on-mutant = kill; tokens, K, chunks logged per mutant</div>
</div>
<p class="muted">The research is in the ablations: the same generation step runs closed-book
(NO_RAG, MuTAP-style) and with retrieved codebase context (RAG) at K = 3/5/8, per mutation
class — so the site can answer whether context helps, where it helps most, how much context
is right, and what a kill costs.</p></div>
...
<p style="text-align:center;margin-top:30px"><a class="btn" href="/research-questions">See the research questions answered →</a></p>
"""
...
(or a <code>--limit</code> smoke run) and reload — this page computes everything from the
real CSVs and never shows placeholder numbers.</div>{% endif %}

{% if d.rq1 %}<h2>RQ1 — Does RAG beat closed-book generation?</h2>
<div class="card"><canvas id="c1" height="110"></canvas>
<p>{{ rq1_take }}</p></div>{% endif %}

{% if d.rq2 %}<h2>RQ2 — Does the benefit vary by mutation class (hardness gradient)?</h2>
<div class="card"><canvas id="c2" height="120"></canvas>
<p>{{ rq2_take }}</p></div>{% endif %}

{% if d.rq3 %}<h2>RQ3 — How much retrieved context is the right amount?</h2>
<div class="card"><canvas id="c3" height="110"></canvas>
<p>{{ rq3_take }}</p></div>{% endif %}
...
<script>
const D = {{ d | tojson }};
if (D.rq1 && Object.keys(D.rq1).length) new Chart(c1, {type:'bar', data:{
  labels:Object.keys(D.rq1), datasets:[{label:'kill rate',
  data:Object.values(D.rq1).map(x=>x.kill_rate), backgroundColor:['#8a8a8a','#2f6bd6']}]},
  options:{scales:{y:{min:0,max:1}},plugins:{legend:{display:false}}}});
if (D.rq2 && D.rq2.classes) new Chart(c2, {type:'bar', data:{labels:D.rq2.classes, datasets:[
  {label:'NO_RAG', data:D.rq2.norag, backgroundColor:'#8a8a8a'},
  {label:'RAG', data:D.rq2.rag, backgroundColor:'#2f6bd6'}]},
  options:{scales:{y:{min:0,max:1,title:{display:true,text:'kill rate'}}}}});
if (D.rq3 && D.rq3.k) new Chart(c3, {type:'line', data:{labels:D.rq3.k, datasets:[
  {label:'kill rate', data:D.rq3.kill_rate, borderColor:'#2f6bd6'},
  {label:'valid test rate', data:D.rq3.valid_test_rate, borderColor:'#2f9e8f'}]},
...
repos (sorts, schedule, and any user-uploaded files) — this deployment has not processed
BugsInPy/HumanEval/MBPP and will not pretend to.</p>
{% if not rows %}<div class="empty">No mutants yet — run the mutate phase first.</div>{% else %}
<div class="card" style="padding:12px 16px">Filter:
...
<h1 class="mono" style="font-size:22px">{{ mid }}</h1>
{% if m %}
<p>{{ badge(m.mutation_class)|safe }} in <code>{{ m.file }}</code> ·
<code>{{ m.function }}</code> (line {{ m.line }}) — {{ m.mutation_description }}</p>
<div class="grid2">
<div><h3>Source with mutation (diff view)</h3>
...
<div><h3>AST (original function; mutated nodes marked)</h3>
<div class="card" style="max-height:480px;overflow:auto">{{ ast_html|safe }}</div></div>
</div>
{% else %}<div class="empty">This mutant is not in the current surviving_mutants.json
(results from an earlier mutant set) — showing its run records only.</div>{% endif %}
...
<span class="badge" style="background:#66708a">generated</span>
...
<span class="badge" style="background:#2f9e8f">{{ c.kind }}</span>
...
across the whole corpus. "KILLED" additionally means it failed on its mutant — the
full kill criterion. The subjects' original suites gate mutant survival upstream and
are not itemized here (doctest-based suites don't decompose into named cases).</p>
{% if not rows %}<div class="empty">No validated tests yet.</div>{% else %}
<input type="text" id="q" placeholder="search mutant / subject / class…"
 onkeyup="f()" style="margin-bottom:12px">
<table class="data" id="tbl"><tr><th>Mutant</th><th>Subject</th><th>Class</th>
...
<td><pre style="max-width:420px;max-height:150px;overflow:auto">{{ r.snippet }}</pre></td>
</tr>{% endfor %}</table>
...
<p>Upload one Python file and describe what it does. CodeKong injects bugs, keeps the
ones a bootstrap suite misses, and returns only tests that provably pass on your code
and fail on a specific bug. <b>This runs a local LLM: expect minutes, not seconds.</b></p>
<div class="card"><form method="post" enctype="multipart/form-data">
<p><label>Python file (.py)<br><input type="file" name="pyfile" accept=".py" required></label></p>
<p><label>What does this code do? (context given to the model)<br>
<textarea name="description" rows="3" required
 placeholder="e.g. Utility functions for clamping and interpolating numeric ranges"></textarea></label></p>
<p style="display:flex;gap:22px;align-items:center">
<label>Mutant cap <input type="text" name="limit" value="15" style="width:70px"></label>
...
<button type="submit">Start generation</button></form></div>
...
<h1>Job {{ j.id[:8] }} — {{ j.filename }}</h1>
<div id="live">
{% if j.status == 'running' %}<div class="empty">Running… started {{ j.age }}s ago.
This page auto-refreshes every 5 s. Local model generation takes minutes per mutant.</div>
<script>setTimeout(()=>location.reload(), 5000)</script>
{% elif j.status == 'error' %}<div class="card"><span class="fail">FAILED</span>
<pre>{{ j.error }}</pre></div>
{% else %}
<div class="statgrid">
<div class="stat"><div class="v">{{ j.report.mutants_attempted }}</div>mutants attempted</div>
<div class="stat"><div class="v">{{ j.report.mutants_killed }}</div>killed → tests kept</div>
<div class="stat"><div class="v">{{ '%.0f%%' % (100*j.report.kill_rate) if j.report.kill_rate is not none else '—' }}</div>kill rate</div>
<div class="stat"><div class="v">{{ '%.0f' % j.report.wall_seconds }}s</div>wall time</div>
<div class="stat"><div class="v">{{ j.report.model }}</div>model</div></div>
...
<p><a class="btn" href="/generate/job/{{ j.id }}/download">Download your test file</a>
<span class="muted" style="margin-left:12px">drops straight into your project; run with pytest</span></p>
<div class="card"><pre>{{ j.preview }}</pre></div>
{% else %}<div class="empty">No mutants were killed, so no test file was kept. Likely causes:
the model tier is too weak, the code needs file/network I/O, or the mutants were equivalent.
